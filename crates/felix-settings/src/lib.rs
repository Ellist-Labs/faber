...
/// `~/.config/felix/settings.toml` on every platform.
pub fn settings_path() -> PathBuf {
    let home = std::env::var("HOME").unwrap_or_else(|_| ".".into());
    PathBuf::from(home).join(".config/felix/settings.toml")
}
...
fn load_from(path: &PathBuf) -> Settings {
    match std::fs::read_to_string(path) {
        Ok(text) => toml::from_str(&text).unwrap_or_else(|err| {
            eprintln!("felix: invalid {}: {err}; using defaults", path.display());
            Settings::default()
        }),
...
mod tests {
    ...
    fn tmp_path(name: &str) -> PathBuf {
        std::env::temp_dir().join(format!("felix_settings_{}_{name}.toml", std::process::id()))
    }
