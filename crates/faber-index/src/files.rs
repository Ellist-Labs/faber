//! Finder data layer: snapshot, entry, query, match, and the `filter()` core.
//!
//! GPUI-free and scan-free — this module owns only the in-memory model and the
//! fuzzy/regex filtering. Project walking lives with its consumer.
...
pub struct FinderQuery {
    ...
    /// Include entries flagged `is_ignored` (gitignored/hidden). Off by default.
...
/// Entries flagged `is_ignored` are excluded unless `q.include_ignored` is set.
pub fn filter(
    ...
    let entry_ok = |e: &FileEntry| {
        (q.include_ignored || !e.is_ignored)
            && match &q.mask {
...
mod tests {
    ...
    #[test]
    fn ignored_entries_excluded_unless_included() {
        let mut snap = snap_of(&["src/lib.rs", "target/out.bin"]);
        for e in &mut snap.entries {
            if e.rel_path.starts_with("target/") {
                e.is_ignored = true;
            }
        }
        // Default query hides ignored entries.
        let hidden = filter(&snap, &q("out"), &[], 100);
        assert!(hidden.is_empty());

        // include_ignored surfaces them.
        let mut query = q("out");
        query.include_ignored = true;
        let shown = filter(&snap, &query, &[], 100);
        assert_eq!(shown.len(), 1);
        assert_eq!(
            snap.entries[shown[0].entry_ix as usize].rel_path,
            "target/out.bin"
        );
    }

    #[test]
    fn ignored_entries_excluded_from_history_unless_included() {
        let mut snap = snap_of(&["src/lib.rs", "target/out.bin"]);
        for e in &mut snap.entries {
            if e.rel_path.starts_with("target/") {
                e.is_ignored = true;
        ...
        let history = vec!["target/out.bin".to_string()];
        assert!(filter(&snap, &q(""), &history, 100).is_empty());

        let mut query = q("");
        query.include_ignored = true;
        let shown = filter(&snap, &query, &history, 100);
        assert_eq!(shown.len(), 1);
    }
